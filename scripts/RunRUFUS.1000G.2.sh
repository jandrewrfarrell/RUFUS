...
#        echo "Skipping build"
#else
#	/usr/bin/time -v $RDIR/cloud/jellyfish-MODIFIED-merge/bin/jellyfish merge $ProbandGenerator.Jhash $ReferenceHash>  $Out.Family.Unique.HashList
#fi
...
then 
	...
	/usr/bin/time -v bash $PullSampleHashes $ProbandGenerator.Jhash <($RDIR/cloud/jellyfish-MODIFIED-merge/bin/jellyfish merge $ProbandGenerator.Jhash $ReferenceHash) $MutantMinCov > $ProbandGenerator.k$K_c$MutantMinCov.HashList
fi 
