				...
				for (int j=0; j<HashSize; j++)
		 		...
		 		if (good)
					...
					if (Mutations.count(LongHash) > 0)
						...
						cout << "found " << LongHash << "\t" << LongToHash(LongHash, 25) << "\t" << Mutations[LongHash]; 
					}
						...
						MutHashesFound++;
                                                positions.push_back(i);
						cout << "found " << LongHash << "\t" << LongToHash(HashToLong(RevComp(hash)), 25) << "\t" << Mutations[HashToLong(RevComp(hash))];	
					}
				}
	                        else
	                        {
