...
int Align3(vector<string>& sequenes, vector<string>& quals, string Ap,
           string Aqp, int Ai, int& overlap, int& index, float minPercentpassed,
           bool& PerfectMatch, int MinOverlapPassed, int Threads) {
...
#pragma omp parallel for shared(index, overlap, bestScore) num_threads(Threads)
  for (int j = start; j < end; j++) {
	...
	if (A.c_str()[k + Acount] == B.c_str()[k + Bcount]) {
	  if (B.c_str()[k + Bcount] != 'N' && (int)Aq.c_str()[k + Bcount] > 5 &&
	      (int)A.c_str()[k + Acount] > 5) {
	    score++;
	  }
	}
    ...
    if (PerfectMatch == false) {
      for (int i = window - 1; i >= MinOverlap; i--) {
        ...
        for (k = 0; k <= i; k++) {
          ...
          if (verbose) {
            cout << "  k = " << k << " so A = " << Alength - i + k
                 << " /\\ B = " << 0 + k << endl;
          }

          if (verbose) {
            cout << "     A >> " << A.c_str()[Alength - i + k - 1] << "="
                 << B.c_str()[0 + k] << " << B" << endl;
          }

          if (A.c_str()[Alength - i + k - 1] == B.c_str()[0 + k]) {
            if (B.c_str()[0 + k] != 'N' &&
                (int)Aq.c_str()[Alength - i + k - 1] > 5 &&
                (int)Bq.c_str()[0 + k] > 5) {
              score++;
            }
          }
  ...
  for (std::vector<string>::size_type i = 0; i < sequenes.size(); i++) {
    ...
    if ((int)i % 10 < 1) {
      ...
      cout << "aligning " << i << " of " << NumReads
           ...
           << ", \% finding match = "
           << ((double)FoundMatch / (double)i) * 100.00 << "\r";
    }
    ...
    int booya = Align3(sequenes, qual, A, Aqual, i, k, bestIndex, MinPercent,
                       PerfectMatch, MinOverlap, Threads);
    if (FullOut) {
