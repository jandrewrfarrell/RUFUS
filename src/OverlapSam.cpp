...
int main (int argc, char *argv[])
{
	...
	while (getline(fastq, L1))
		...
		bool b[16];
		int v = atoi(temp[1].c_str());  // number to dissect
		for (int j = 0;  j < 16;  ++j)
   			b [j] =  0 != (v & (1 << j));

		if (b[8] or b[11] or b[10] or temp[10].length() < 100 )	
		{
			//cout << "REJECTED: " << endl << L1 << endl;	
	 ...
	 for (int i = 0; i < Unsequenes.size(); i++)
        {
                ...
                if(FullOut){cout << ">>>>" << Unsequenes[i] << endl;}
                if (Unsequenes[i] !="moved" && Unsequenes[i].size() >= 95)
                {
				int maxDep = -1; 
                                if (Unsequenes[i].size() !=  Unqual[i].size() &&  Unqual[i].size() != Undepth[i].size()){cout << "ERROR, read " << "@NODE_" << argv[6] << "_" << i << "_L=" <<Unsequenes[i].size() << "_D=" << maxDep << " Has the wrong size, Seq = " << Unsequenes[i].size() << " Qual = " << Unqual[i].size() << " Dep = " <<  Undepth[i].size() << endl;}
                                count ++;
                                report << "@NODE_" << argv[6] << "_" << i << "_L=" <<Unsequenes[i].size() << "_D" << maxDep <<  endl;
                                report << Unsequenes[i] << endl;
                                report << "+" << endl;
                                report << Unqual[i] << endl;

                                Depreport << "@NODE_" << argv[6] << "_" << i <<  "_L=" << Unsequenes[i].size() << "_D" << maxDep << endl;
