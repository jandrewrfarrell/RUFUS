...
void SamRead::parseMutations()
{
	...
	for(int i = 0; i<cigarString.size(); i++)
	{
		...
		alt = "";
		varType = ""; 
		if ((cigarString.c_str()[i] == 'X' or cigarString.c_str()[i] == 'I' or cigarString.c_str()[i] == 'D' /* or cigarString.c_str()[i] == 'S' or cigarString.c_str()[i] == 'H'*/) and RefSeq.c_str()[i] != 'N')
		{
			int size = -1; 
			int startPos = i; 
			...
			{
				if ( cigarString.c_str()[i] == 'I' or cigarString.c_str()[i] == 'D' /*or cigarString.c_str()[i] == 'S' or cigarString.c_str()[i] == 'H'*/)
				{
...
//					if (reads[A].ChrPositions[i] == LastAlignedChr )
					if (reads[A].chr == reads[B].chr )
					{
						...
						BEDBigStuff << reads[A].chr << "\t" << reads[A].Positions[i] << "\t" << LastAlignedPos << "\tInsertion" << endl;
						cout << "tadem dup" << endl;
						int j = 0; 
	...
	//						cout << "R " << '-' << " " << 'D' << " " << tmp << " " << reads[B].chr << " " << j << " " << NewRef << endl << endl;
				...
				else if (reads[B].Positions[i] -LastAlignedPos < 0 and abs(reads[B].Positions[i] - LastAlignedPos ) < MaxVarentSize) // indicates a possible insertion or tandem duplication
				{
					...
					if (reads[B].chr == reads[A].chr  )
					{
						...
						BEDBigStuff << reads[B].chr << "\t" << reads[B].Positions[i] << "\t" << LastAlignedPos << "\tInsertion" << endl;
					//	cout << "tadem dup" << endl;
						int j = 0; 
...
";
	...
	VCFOutFile << "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\tSample"<< endl;
