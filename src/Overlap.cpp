...
int RebuildHashTable(vector<string>& sequenes, int Ai, int SearchHash,
                     unordered_map<unsigned long, vector<int>>& Hashes,
                     int Threads) {
  cout << "\nDestrying HashTable\n";
  Hashes.clear();
...
int PrepairSearchList(string A, int Ai,
                      unordered_map<unsigned long, vector<int>>& Hashes,
                      int SearchHash, int ACT, map<int, vector<int>>& array,
                      bool& hitPosLimit, bool& hitIndexLimit, int& NumberPos,
                      int& NumberIndex) {
  int Alength = A.size();
...
int Align3(vector<string>& sequenes, string Ap, string Aq, int Ai, int& overlap,
           int& BestIndex, float minPercent, bool& PerfectMatch, int MinOverlap,
           vector<int>& indexes, int Threads, int NumReads) {
  ...
  for (int booya = 0; booya < indexes.size(); booya++) {
    ...
    if (PerfectMatch == false) {
      for (int i = window - 1; i >= MinOverlap; i--) {
        ...
        for (k = 0; k <= i; k++) {
          if (verbose) {
            cout << "  k = " << k << " so A = " << AlengthL - i + k
                 << " /\\ B = " << 0 + k << endl;
          }
          if (verbose) {
            cout << "     A >> " << A.c_str()[AlengthL - i + k - 1] << "="
                 << B.c_str()[0 + k] << " << B" << endl;
          }
...
#pragma omp parallel for num_threads(Threads) shared(Hashes, Forwards)
    for (int i = b; i < max; i++) {
      ...
      int NumPos = 0;
      int NumSanity = 0;
      PrepairSearchList(A, i, Hashes, SearchHash, ACT, Forwards, posLimit,
                        sanityLimit, NumPos, NumSanity);
      if (posLimit) {
        NumberHitPosLimit++;
      }
      if (sanityLimit) {
      ...
      AverageFPos =
	((AverageFPos * (double)b) + (double)NumPos) / ((double)b + 1.0);
      AverageFSanity = ((AverageFSanity * (double)b) + (double)NumSanity) /
	((double)b + 1.0);
    }
...
#pragma omp parallel for num_threads(Threads) shared(Hashes, Revs)
    for (int i = b; i < max; i++) {
      ...
      int NumPos = 0;
      int NumSanity = 0;
      PrepairSearchList(A, i, Hashes, SearchHash, ACT, Revs, posLimit,
                        sanityLimit, NumPos, NumSanity);
      if (posLimit) {
        NumberHitPosLimit++;
      }
      ...
      AverageRPos =
	((AverageRPos * (double)b) + (double)NumPos) / ((double)b + 1.0);
      AverageRSanity = ((AverageRSanity * (double)b) + (double)NumSanity) /
	((double)b + 1.0);
    }
    ...
    for (int i = b; i < max; i++) {
      ...
      int booya =
	Align3(sequenes, A, Aqual, i, k, bestIndex, MinPercent, PerfectMatch,
	       MinOverlap, Forwards[i], Threads, NumReads);
      ...
      if (!(PerfectMatch)) {
        ...
        int revbooya =
	  Align3(sequenes, revA, revAqual, i, revk, revbestIndex, MinPercent,
		 PerfectMatch, MinOverlap, Revs[i], Threads, NumReads);
        if (FullOut) {
...
#pragma omp critical
            ...
            for (k = 0;
                 ...
                 k++) {
              if (Hashes[Util::HashToLong(Util::RevComp(hash))][k] ==
                  bestIndex) {
                found = true;
                break;
              }
...
#pragma omp critical
              {
                Hashes[Util::HashToLong(Util::RevComp(hash))].push_back(
									bestIndex);
              }
            }
          }
