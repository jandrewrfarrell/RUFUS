...
then 
	...
	/usr/bin/time -v bash $PullSampleHashes $ProbandGenerator.Jhash Family.Unique.HashList $MutantMinCov > $ProbandGenerator.k$K_c$MutantMinCov.HashList
fi 
...
then 
	...
	/usr/bin/time -v bash $PullSampleHashes $SiblingGenerator.Jhash Family.Unique.HashList $SiblingMinCov > $SiblingGenerator.k$K_c$SiblingMinCov.HashList
fi 
